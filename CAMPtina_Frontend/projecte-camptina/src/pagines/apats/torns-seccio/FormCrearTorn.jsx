...
export const FormCrearTorn = () => {
    ...
    const peticioCrearTorn = handleSubmit((data) => {
        ...
        const nouTorn = {
            nom: data.nomDeTorn,
            aforament_maxim: parseInt(data.aforament),
            hora_inici: data.horaInici,
            hora_final: data.horaFinal
        };
        console.log("Torn creat: ", nouTorn);

        const tornsGuardats = JSON.parse(localStorage.getItem('torns')) || [];
        tornsGuardats.push(nouTorn);
        localStorage.setItem('torns', JSON.stringify(tornsGuardats));
                    ...
                    <div className={className_div_txt_form}>
                        ...
                        <label htmlFor={id_input_txt}>{txtInputhoraFinal}</label>
                            <input
                                ...
                                { ... register('horaFinal', {
                                    required: true,
                                    validate: value => !horaInici || value > horaInici
                                })}
                            >
                            </input>
                        </div>
                        ...
                        <span className={className_span}>Hora final obligatòria</span> }
                        { errors.horaFinal?.type === 'validate' &&
                        <span className={className_span}>La hora final ha de ser posterior a la d'inici</span> }
                        </div>
                    </div>
                </div>
